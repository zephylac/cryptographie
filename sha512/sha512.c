...
{
	WORD a, b, c, d, e, f, g, h, i, j, t1, t2, m[128];

	for (i = 0, j = 0; i < 16; ++i, j += 8)
		m[i] = ((WORD)ctx->data[j] << 56) |  ((WORD)ctx->data[j+1] << 48) |  ((WORD)ctx->data[j+2] << 40) | (((WORD)ctx->data[j+3] << 32) | (WORD)ctx->data[j+4] << 24) | ((WORD)ctx->data[j + 5] << 16) | ((WORD)ctx->data[j + 6] << 8) | ((WORD)ctx->data[j + 7]);
	for ( ; i < 80; ++i)
...
void sha512_compute(SHA512_CTX *ctx, const BYTE data[], size_t len)
{
	...
	ctx->data[119] = ctx->bitlen2;
	ctx->data[118] = ctx->bitlen2 >> 8;
	ctx->data[117] = ctx->bitlen2 >> 16;
	ctx->data[116] = ctx->bitlen2 >> 24;
	ctx->data[115] = ctx->bitlen2 >> 32;
	ctx->data[114] = ctx->bitlen2 >> 40;
	ctx->data[113] = ctx->bitlen2 >> 48;
	ctx->data[112] = ctx->bitlen2 >> 56;
	sha512_compress(ctx);
}
